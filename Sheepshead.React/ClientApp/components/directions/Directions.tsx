...
export class Directions extends React.Component<RouteComponentProps<{}>, DirectionsState> {
    ...
    private _inputNodes: { [elementName: string]: HTMLDivElement | null } = {};
    ...
    private onLinkClick(e: any) {
        ...
        var elem = this._inputNodes[hash];
        if (elem !== null)
            elem.scrollIntoView();
    }
    ...
    private renderNavigationButtons(rowOneCount: number, rowTwoCount: number) {
        ...
        for (var buttonNumber = 1; buttonNumber < rowOneCount; ++buttonNumber)
            rowOneButtons.push(<a className='jump-button' href='directions#tricks' onClick={this.onLinkClick} key={buttonNumber}>{buttonNumber}</a>);
        ...
        for (var buttonNumber = rowOneCount+1; buttonNumber <= rowOneCount + rowTwoCount; ++buttonNumber)
            rowTwoButtons.push(<div className='jump-button' key={buttonNumber}>{buttonNumber}</div>);
    ...
    private renderSlide(slideName: string, slideContent: JSX.Element) {
        return (
            <div className='slide' name={slideName} ref={node => this._inputNodes[slideName] = node}>
